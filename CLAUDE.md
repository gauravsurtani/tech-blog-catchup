...
python run.py crawl --mode full           # full sitemap archive crawl, all sources
...
### Frontend (Node 18+, from `frontend/` directory)
...
- **`crawler/`** — Two crawl modes: `crawl_full` (sitemap discovery via `sitemap_parser.py` + Crawl4AI batch extraction) and `crawl_incremental` (RSS via `feed_parser.py` + `atoma` + Crawl4AI for full text). `crawl_manager.py` orchestrates both.
...
- **Global audio player** — `AudioPlayerProvider` context in `hooks/useAudioPlayer.ts` wraps the entire app via `layout.tsx`. Persistent `<audio>` element with queue/history/seek/volume/playbackRate state. The `AudioPlayer` component renders as a fixed bottom bar with speed control (0.5x-3x) and keyboard shortcuts (Space=play/pause, M=mute).
...
- **Crawl modes**: "incremental" (default, RSS feeds, fast) vs "full" (sitemaps, slow, gets entire archive).
...
|--------|------|-------------|
| GET | `/api/posts` | Paginated posts (filter: source, tag, search, audio_status, sort) |
